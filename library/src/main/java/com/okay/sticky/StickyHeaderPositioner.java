...
final class StickyHeaderPositioner {
    ...
    private void matchMarginsToPadding(MarginLayoutParams layoutParams) {
        int leftMargin = orientation == LinearLayoutManager.VERTICAL ?
                recyclerView.getPaddingLeft() : 0;
        int topMargin = orientation == LinearLayoutManager.VERTICAL ?
                (int) recyclerView.getY() : recyclerView.getPaddingTop();
        int rightMargin = orientation == LinearLayoutManager.VERTICAL ?

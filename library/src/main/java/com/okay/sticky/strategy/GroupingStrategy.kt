package ccom.okay.sticky.strategy

import android.support.v7.widget.RecyclerView
import com.okay.sticky.callback.Action
import com.okay.sticky.callback.BinaryCondition
import com.okay.sticky.callback.Condition
...
class GroupingStrategy<T> {
    private val adapter: StickyCallback<T>
    ...
    constructor(adapter: StickyCallback<T>) {
        this.adapter = adapter
        this.indexItems = ArrayList<Int>()
        if(adapter is RecyclerView.Adapter<*>) {
            registerAdapterDataObserver(adapter as RecyclerView.Adapter<RecyclerView.ViewHolder>)
        }else{
            throw IllegalArgumentException("adapter is  must a RecyclerView.Adapter<RecyclerView.ViewHolder>")
        }
    }
     ...
     * @param adapter
     */
    private fun registerAdapterDataObserver(adapter: RecyclerView.Adapter<RecyclerView.ViewHolder>) {
        //同步整个列表数据变化
    ...
    companion object {
        fun <T> of(adapter: StickyCallback<T>): GroupingStrategy<T> {
